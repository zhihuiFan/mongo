...
void WiredTigerRecoveryUnit::_txnClose(bool commit) {
    ...
    if (commit) {
        ...
        log() << "WT commit_transaction for snapshot id " << _mySnapshotId;
    } else {
        invariantWTOK(s->rollback_transaction(s, NULL));
        log() << "WT rollback_transaction for snapshot id " << _mySnapshotId;
    }
    _active = false;
    _mySnapshotId = nextSnapshotId.fetchAndAdd(1);
...
void WiredTigerRecoveryUnit::_txnOpen(OperationContext* opCtx) {
    ...
    log() << "WT begin_transaction for snapshot id " << _mySnapshotId;
    _timer.reset();
    _active = true;
}
